...
void save_leaderboard(timer last_time){
    pointer = fopen("Leaderboard.txt","a");
    i++;
    fprintf(pointer,"%d %zu", i, last_time);
    fclose(pointer);
}

void read_leaderboard(){
   if ((pointer = fopen("Leaderborad.txt","r")) == NULL){
       printf("Error! Leaderboard not read!");
   }else{
       for(i=0;i<10;i++){
           fscanf(pointer,"%d %d", &matriz[i][1], &matriz[i][2]);
       }
   }
}
